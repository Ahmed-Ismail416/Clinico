...
namespace Services
{
    public class AppointmentService(IUnitOfWork _unitOfWork) : IAppointmentService
    {
        ...
        #region GetAllApointmentAsync
        public async Task<PaginationResult<AppointmentResponseDto>> GetAllApointmentAsync(AppointmentParams ap)
        {
            var spec = new AppointmentWithDetailsSpecification(ap);
            var appointments = await _unitOfWork.GetRepo<Appointment, int>().ListAsyncWithSpec(spec);
            var appointmentsData= appointments.Adapt<IReadOnlyList<AppointmentResponseDto>>();
            var specCount = new AppointmentCountSpecification(ap);
            var appointmentCount = await _unitOfWork.GetRepo<Appointment, int>().CountAsync(specCount);
            var result = new PaginationResult<AppointmentResponseDto>(ap.PageIndex, ap.PageSize, appointmentCount, appointmentsData);
            return result;
        }
        #endregion


        #region GetAppointmentsByIdAsync
        ...
        #region GetMyAppointmentAsync
        public async Task<IReadOnlyList<AppointmentResponseDto>> GetMyAppointmentAsync(string doctorId, string role)
        {

            var spec = new AppointmentFilteringSpecification(doctorId.ToString(), role);
            var appointments = await _unitOfWork.GetRepo<Appointment, int>().ListAsyncWithSpec(spec);
        ...
        public async Task<AppointmentResponseDto> UpdateAppointmentAsync(int id, UpdateAppointmentDto dto)
        {
            ...
            var conflictSpec = new AppointmentConflictSpecification(appointment.DoctorId, dto.StartTime, dto.EndTime, id);

            var conflictingAppointments = await _unitOfWork.GetRepo<Appointment, int>().CountAsync(conflictSpec);
            if(conflictingAppointments > 0)
                throw new AppointmentConflictException("The doctor is not available during the requested time slot.");
        ...
        #region UpdateAppointmentSatusAsync
